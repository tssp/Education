...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.groupBy{ c:Char => c }.map{ m => (m._1, m._2.size) }.toList.sortBy{ _._1 }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =  {
    ...
    val result = occurrences match {
      ...
      // o  -> (Char, Int)
      // os -> List((Char, Int))
      case o::os => if(o._2 == 0) Nil
                    else {
                      
                      val current= (1 to o._2).map { count => List((o._1, count)) }.toList
                      val next= combinations(os).filter { _ != Nil}
                      val cross= current.flatMap { c => next.map{ n => c:::n } } 
